...
type vectorsFile struct {
	...
	DkgEncShare  []dkgEncShareVec   `json:"dkgEncShare"`
}

type dkgEncShareVec struct {
	Description    string   `json:"description"`
	CommitmentsHex []string `json:"commitmentsHex"`
	J              uint32   `json:"j"`
	PkRHex         string   `json:"pkRHex"`
...
// TestVectors_DkgEncShare verifies a cross-language test vector: a proof
// generated by the TypeScript primitive must verify byte-for-byte under the
// Go verifier. If this test ever breaks it means the Go and TS
// implementations have diverged — transcript framing, canonical encodings,
// or SHA-512 folding must be investigated immediately.
func TestVectors_DkgEncShare(t *testing.T) {
	vf := loadVectors(t)
	if len(vf.DkgEncShare) == 0 {
		t.Fatalf("expected at least one dkgEncShare vector")
	}
	for i, v := range vf.DkgEncShare {
		commitments := make([]Point, 0, len(v.CommitmentsHex))
		for k, ch := range v.CommitmentsHex {
			c, err := PointFromBytesCanonical(mustHex(t, ch))
			if err != nil {
				t.Fatalf("dkgEncShare vec[%d] C[%d]: %v", i, k, err)
			}
			commitments = append(commitments, c)
		}
		pkR, err := PointFromBytesCanonical(mustHex(t, v.PkRHex))
		if err != nil {
			t.Fatalf("dkgEncShare vec[%d] pkR: %v", i, err)
		}
		U, err := PointFromBytesCanonical(mustHex(t, v.UHex))
		if err != nil {
			t.Fatalf("dkgEncShare vec[%d] U: %v", i, err)
		}
		V, err := PointFromBytesCanonical(mustHex(t, v.VHex))
		if err != nil {
			t.Fatalf("dkgEncShare vec[%d] V: %v", i, err)
		}
		proofBytes := mustHex(t, v.ProofHex)
		p, err := DecodeDkgEncShareProof(proofBytes)
		if err != nil {
			t.Fatalf("dkgEncShare vec[%d] decode: %v", i, err)
		}
		ok, err := DkgEncShareVerify(commitments, v.J, pkR, U, V, p)
		if err != nil {
			t.Fatalf("dkgEncShare vec[%d] verify err: %v", i, err)
		}
		if !ok {
			t.Fatalf("dkgEncShare vec[%d] (%s) cross-lang verify failed", i, v.Description)
		}

		// Encode/decode round-trip must be byte-stable.
		re := EncodeDkgEncShareProof(p)
