...
### 2026-06-19T~UTC (latest) — Tests for the Untested Authoritative Hand Evaluator (Go `internal/holdem` + `internal/cards`)
Maintenance pass (local-only; no deploys). Repo started green/clean. Oriented across the monorepo, confirmed the prior session's flagged "shuffle step 4/3" finding is already fixed (commit `1713655` added the `actualStep >= epochMembers.length` guard in `apps/dealer-daemon/src/handlers/shuffle.ts:46`, capping the round at N). Manually verified the TS hand evaluator (`packages/holdem-eval`, has a 450-hand pokersolver cross-check), the TS side-pot/engine logic, and the Go settlement path (`x/poker/keeper/logic.go` `computeSidePots`/`settleKnownShowdown`) — all correct. A dispatched subagent cleared the TS `poker-engine` state machine.

**The gap (highest-value, lowest-risk):** the **authoritative on-chain hand evaluator that awards real chips at showdown** — Go `apps/cosmos/internal/holdem/eval.go` (`Evaluate7`/`Winners`/`evaluate5`/`CompareHandRank`) — had **ZERO unit tests**, as did its dependency `apps/cosmos/internal/cards`. (Its TS twin is heavily cross-checked; the Go one that actually settles was unguarded.)

**What landed (2 new test files + 1 cosmetic cleanup):**
- `apps/cosmos/internal/holdem/eval_test.go` (574 LoC): known-answer tests for all 9 categories + tiebreakers (incl. steel wheel, kicker-as-third-element two-pair `[13,2,14]`); a strict 18-rung weakest→strongest `CompareHandRank` ladder (reflexive/antisymmetric); `Evaluate7` best-of-7 selection traps (slides to higher straight, flush among 7, two-trips→full house, wheel-SF not upgraded by off-suit honors); `Winners` cases (single, board-plays chop, kicker, **six-high straight beats the wheel**, out-of-range seat filtering, error paths); determinism guard (Go map-order); a `combos7Choose5` completeness check. Plus two oracles: **(a) exhaustive `C(52,5)=2,598,960` category-distribution test** matching the standard poker frequencies (1302540/1098240/123552/54912/10200/5108/3744/624/40) — an independent ground-truth that catches essentially any miscategorization (~1.0s, skipped under `-short`); **(b) a 5000-deal fuzz** cross-checking `Evaluate7` against an independent drop-2-of-7 best-five oracle and `Winners` against re-derived tie sets (seeded RNG, deterministic).
- `apps/cosmos/internal/cards/cards_test.go` (119 LoC): `Rank()`/`Suit()` for all 52, `String()` known values + full-deck format/uniqueness, `DeterministicDeck` permutation + determinism.
- `apps/cosmos/internal/cards/cards.go`: simplified `Card.String()` — dropped a redundant `default` branch immediately overwritten by an `if`, plus a no-op `(r-0)`. Behavior byte-identical for all ranks 2..14 (the `default` only ever sees 2..9 since `Rank()∈[2,14]` and 10..14 have explicit cases); now pinned by the new tests. Events aren't part of the app-hash, so zero replay/consensus risk regardless.

**Verification:** `cd apps/cosmos && go test ./...` green end-to-end (new `internal/cards` + `internal/holdem` pass; keeper/dealer suites unaffected — only test files + one cosmetic line touched); `go build ./...` clean; `go vet ./internal/...` clean; `gofmt` clean. Two independent code-review subagents returned **no findings**: one re-derived every hard-coded expected value from poker first principles + closed-form combinatorics (no wrong expectations locked in); the other cleared the harness for flakiness (fixed seeds, no map-order reliance, safe deck-dealing bounds `6*2+5=17 < 52`, genuinely independent oracle) and confirmed the `String()` refactor is behavior-preserving. The exhaustive oracle **passing** is strong evidence the Go evaluator was already correct — this pass makes that a permanent regression guard. No TS/chain-consensus code touched; no deploy.

### 2026-06-18T~UTC — Web Hole-Card Recovery Unbroken + Bot Short-Stack Raise + Test-Orchestration Gaps
...
- **TWO parallel hand evaluators — keep them semantically identical**: the authoritative one that settles real chips at showdown is **Go** `apps/cosmos/internal/holdem/eval.go` (`Winners`/`Evaluate7`/`evaluate5`/`CompareHandRank`), called from `x/poker/keeper/logic.go:settleKnownShowdown`. The TS `packages/holdem-eval` is its mirror, used off-chain by bots/sim/web and cross-checked against `pokersolver` (450 random hands). Both share the same conventions: tiebreakers high→low; wheel (A-2-3-4-5) is a 5-high straight `[5]`; two-pair tiebreaker is `[highPair, lowPair, kicker]` (kicker can exceed the low pair, e.g. `KK 22 A` → `[13,2,14]`); category order HighCard<…<StraightFlush. The Go side is now guarded by `internal/holdem/eval_test.go`, whose exhaustive `C(52,5)` category-distribution test is an independent ground-truth oracle — if you change either evaluator, re-run it and keep the two in lockstep.
